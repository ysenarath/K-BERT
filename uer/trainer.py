...
def worker(gpu_id, gpu_ranks, args, model):
    ...
    if gpu_ranks is None:
        train_loader = globals()[args.target.capitalize() + "DataLoader"](
            args, args.dataset_path, args.batch_size, gpu_id, 1, True
        )
    else:
        train_loader = globals()[args.target.capitalize() + "DataLoader"](
            args, args.dataset_path, args.batch_size, gpu_id, len(gpu_ranks), True
        )
    ...
    globals().get("train_" + args.target)(
        args, gpu_id, rank, train_loader, model, optimizer
    )
...
def train_bert(args, gpu_id, rank, loader, model, optimizer):
    ...
    total_correct_mlm, total_denominator = 0.0, 0.0
    # Calculate NSP accuracy.
    total_correct_nsp, total_instances = 0.0, 0.0
    steps = 1
...
def train_bilm(args, gpu_id, rank, loader, model, optimizer):
    model.train()
    start_time = time.time()
    total_loss, total_loss_forward, total_loss_backward = 0.0, 0.0, 0.0
    # Calculate BiLM accuracy.
    total_correct_forward, total_correct_backward, total_denominator = 0.0, 0.0, 0.0
    steps = 1
    total_steps = args.total_steps
    loader_iter = iter(loader)
    ...
    while True:
        ...
        if steps % args.report_steps == 0 and (
            ...
            total_loss, total_loss_forward, total_loss_backward = 0.0, 0.0, 0.0
            total_correct_forward, total_correct_backward, total_denominator = (
                0.0,
                0.0,
                0.0,
            )
...
def train_cls(args, gpu_id, rank, loader, model, optimizer):
    ...
    start_time = time.time()
    total_loss = 0.0
    total_correct, total_instances = 0.0, 0.0
    steps = 1
    total_steps = args.total_steps
    loader_iter = iter(loader)
